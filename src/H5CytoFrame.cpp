...
EVENT_DATA_VEC H5CytoFrame::read_data(uvec col_idx) const {
  ...
  for (unsigned i = 0; i < ncol; i++) {
    ...
    dataset.read(data.memptr(), h5_datatype_data(DataTypeLocation::MEM),
                 memspace, dataspace);
  }
...
/*
 * for simplicity, we don't want to handle the object that has all the h5
 * handler closed because it will require lots of validity checks before each
 * disk IO operations
 */
//	void close_h5(){
//		dataspace.close();
...
void H5CytoFrame::load_meta() {
  ...
  /*
   * have to redefine cytoParam to use char * since H5 can't directly read into
   * string member of the compound type
   */
  struct cytoParam1 {
    char *channel;
    char *marker;
  ...
  param_type.insertMember("PnB", HOFFSET(cytoParam1, PnB),
