...
transformation::transformation()
    : isGateOnly(false), isDataOnly(false), type(CALTBL), isComputed(true) {}
transformation::transformation(bool _isGate, unsigned short _type)
    : isGateOnly(_isGate), isDataOnly(false), type(_type), isComputed(true) {}
...
Spline_Coefs transformation::getSplineCoefs() {
  return calTbl.getSplineCoefs();
};
void transformation::setCalTbl(calibrationTable _tbl) { calTbl = _tbl; }
...
TransPtr transformation::clone() const {
  return TransPtr(new transformation(*this));
};
transformation::transformation(const pb::transformation &trans_pb) {
  ...
  /* For PB_BIEXP, caltbl was not saved during archiving and thus could be 0x0
    and thus trans_pb.caltbl() call is supposed to fall back to return the one
    from default_instance, which should not be null. However strange enough, in
    some circumstances (e.g. after save_gs() call), this
    default_instance_->caltbl_ does become null which leads this pb auto
    generated accessor function to be unsafe to be invoked. So we have to skip
    it in case of biexp. (we don't need to do it anyway)
  */
...
void transformation::convertToPb(pb::transformation &trans_pb) {
  ...
  /*skip saving calibration table to save disk space, which means it needs to be
     always recalculated when load it back Setting the flag to FALSE can only
     make sure it is recomputed properly for the APIs where the flag is checked
     first but it is not sufficient to prevent the segfault because the pointer
     to the caltbl in pb object is unset and somehow the
          default_instance_->caltbl_ can also be null due to some previous
     operations (i.e.`save_gs`). So ::pb::calibrationTable&
     transformation::caltbl becomes unsafe to call.
  */
  if (type == BIEXP)
...
TransPtr transformation::getInverseTransformation() {
  ...
  // it returns the base transformation type instead of the original one (e.g.
  // biexp)
  inverse->type = CALTBL;
...
void transformation::setTransformedScale(int scale) {
  throw(domain_error("setTransformedScale function not defined!"));
};
int transformation::getTransformedScale() {
  throw(domain_error("getTransformedScale function not defined!"));
};
int transformation::getRawScale() {
  throw(domain_error("getRawScale function not defined!"));
};
...
biexpTrans::biexpTrans()
    : transformation(false, BIEXP), channelRange(4096), pos(4.5), neg(0),
      widthBasis(-10), maxValue(262144) {
  setComputeFlag(false);
  calTbl.setInterpolated(false);
}

biexpTrans::biexpTrans(int channelRange_, EVENT_DATA_TYPE pos_,
                       EVENT_DATA_TYPE neg_, EVENT_DATA_TYPE widthBasis_,
                       EVENT_DATA_TYPE maxValue_)
    : transformation(false, BIEXP), channelRange(channelRange_), pos(pos_),
      neg(neg_), widthBasis(widthBasis_), maxValue(maxValue_) {
  setComputeFlag(false);
  calTbl.setInterpolated(false);
}
...
void biexpTrans::computCalTbl() {
  ...
  if (zeroChan > 0)
    decades = extra * channelRange / zeroChan;
  width /= 2 * decades; // 1.1
  ...
  int nPoints = maxChannlVal; // 4097;//fix the number of points so that it
                              // won't lost the precision when scale is set to
                              // 256 (i.e. channelRange = 256)
  ...
  EVENT_DATA_TYPE s =
      exp((positiveRange + negativeRange) * (width + extra / decades));
  for (int j = 0; j < nPoints; j++)
    negative[j] *= s;
  ...
  for (int j = 0; j < zeroChan; j++) {
    int m = 2 * zeroChan - j;
    if (m < 0 || m >= nPoints)
      throw(logic_error("invalid value from '2 * zeroChan - j': " +
                        std::to_string(m)));
...
biexpTrans::biexpTrans(const pb::transformation &trans_pb)
    : transformation(trans_pb) {
  if (!trans_pb.has_bt())
  ...
  // make sure to always recompute caltbl (regardless of compute flag) since it
  // was not saved for the sake of space
  //	computCalTbl(); //now we do lazy-compute since some global trans (in the
  //legacy ws and these trans are not actually used by any samples) may not have
  //the valid parameters which will fail the pb unarchiving process
}
...
fasinhTrans::fasinhTrans()
    : transformation(false, FASINH), maxRange(262144), length(256), T(262144),
      A(0), M(4.5) {
  calTbl.setInterpolated(true);
}

fasinhTrans::fasinhTrans(EVENT_DATA_TYPE _maxRange, EVENT_DATA_TYPE _length,
                         EVENT_DATA_TYPE _T, EVENT_DATA_TYPE _A,
                         EVENT_DATA_TYPE _M)
    : transformation(false, FASINH), maxRange(_maxRange), length(_length),
      T(_T), A(_A), M(_M) {
  calTbl.setInterpolated(true);
}
/*
...
void fasinhTrans::transforming(EVENT_DATA_TYPE *input, int nSize) {
  ...
  for (int i = 0; i < nSize; i++) {
    input[i] = length *
               (asinh(input[i] * sinh(M * log(10)) / T) + A * log(10)) /
               ((M + A) * log(10));
  }
  ...
  //		input = input / myA;
  //
  //		// This formula for the arcsinh loses significance when x is
  //negative
  //		//Therefore we take advantage of the fact that sinh is an odd
  //function 		input = abs(input);
  //
...
TransPtr fasinhTrans::clone() const {
  return TransPtr(new fasinhTrans(*this));
};
...
fasinhTrans::fasinhTrans(const pb::transformation &trans_pb)
    : transformation(trans_pb) {
...
fsinhTrans::fsinhTrans(EVENT_DATA_TYPE _maxRange, EVENT_DATA_TYPE _length,
                       EVENT_DATA_TYPE _T, EVENT_DATA_TYPE _A,
                       EVENT_DATA_TYPE _M)
    : fasinhTrans(_maxRange, _length, _T, _A, _M) {}

void fsinhTrans::transforming(EVENT_DATA_TYPE *input, int nSize) {
  for (int i = 0; i < nSize; i++)
    input[i] = sinh(((M + A) * log(10)) * input[i] / length - A * log(10)) * T /
               sinh(M * log(10));
}
TransPtr fsinhTrans::getInverseTransformation() {
  throw(domain_error("inverse function not defined!"));
};
...
logTrans::logTrans()
    : transformation(false, LOG), offset(0), decade(1), scale(1), T(262144) {
  calTbl.setInterpolated(true);
}

logTrans::logTrans(EVENT_DATA_TYPE _offset, EVENT_DATA_TYPE _decade,
                   unsigned _scale, unsigned _T)
    : transformation(false, LOG), offset(_offset), decade(_decade),
      scale(_scale), T(_T) {
  calTbl.setInterpolated(true);
}
...
logTrans::logTrans(const pb::transformation &trans_pb)
    : transformation(trans_pb) {
...
logInverseTrans::logInverseTrans(EVENT_DATA_TYPE _offset,
                                 EVENT_DATA_TYPE _decade, unsigned _scale,
                                 unsigned _T)
    : logTrans(_offset, _decade, _scale, _T){};
...
logGML2Trans::logGML2Trans(EVENT_DATA_TYPE _T, EVENT_DATA_TYPE _M)
    : transformation(false, LOGGML2), T(_T), M(_M) {
  calTbl.setInterpolated(true);
}
...
void logGML2Trans::transforming(EVENT_DATA_TYPE *input, int nSize) {
  ...
  if (!min) {
    ...
    if (nSize > 1)
      throw(domain_error("All data values are negative. Cannot impute minimum "
                         "value for GML2 log transform."));
  }
...
TransPtr logGML2Trans::clone() const {
  return TransPtr(new logGML2Trans(*this));
};
...
logGML2Trans::logGML2Trans(const pb::transformation &trans_pb)
    : transformation(trans_pb) {
...
logGML2InverseTrans::logGML2InverseTrans(EVENT_DATA_TYPE _T, EVENT_DATA_TYPE _M)
    : logGML2Trans(_T, _M){};
...
linTrans::linTrans(const pb::transformation &trans_pb)
    : transformation(trans_pb) {}
TransPtr linTrans::getInverseTransformation() {
  throw(domain_error("inverse function not defined!"));
};
void linTrans::setTransformedScale(int scale) {
  throw(domain_error("setTransformedScale function not defined!"));
};

scaleTrans::scaleTrans()
    : transformation(true, SCALE), t_scale(256), r_scale(262144) {
  calTbl.setInterpolated(true);
  scale_factor = t_scale / (EVENT_DATA_TYPE)r_scale;
}
scaleTrans::scaleTrans(int _t_scale, int _r_scale)
    : transformation(true, SCALE), t_scale(_t_scale), r_scale(_r_scale) {
  calTbl.setInterpolated(true);
  if ((_r_scale == 0) || (_t_scale == 0))
    throw(domain_error("Illegal arguments provided to scaleTrans constructor: "
                       "t_scale and r_scale must be nonzero"));
  scale_factor = t_scale / (EVENT_DATA_TYPE)r_scale;
}

// Defines a scaleTrans solely on float scale factor (not as a ratio of ints)
scaleTrans::scaleTrans(EVENT_DATA_TYPE _scale_factor)
    : transformation(true, SCALE), t_scale(1), r_scale(1),
...
scaleTrans::scaleTrans(const pb::transformation &trans_pb)
    : transformation(trans_pb) {
...
TransPtr scaleTrans::getInverseTransformation() {
  if (t_scale == 0)
    return TransPtr(
        new scaleTrans(1.0 / scale_factor)); // just invert the scale factor
  else
    return TransPtr(
        new scaleTrans(r_scale, t_scale)); // swap the raw and trans scale
}
...
flinTrans::flinTrans(EVENT_DATA_TYPE _minRange, EVENT_DATA_TYPE _maxRange)
    : transformation(false, FLIN), min(_minRange), max(_maxRange) {
  calTbl.setInterpolated(true);
}
...
flinTrans::flinTrans(const pb::transformation &trans_pb)
    : transformation(trans_pb) {
  const pb::flinTrans &ft_pb = trans_pb.flt();
  max = ft_pb.max();
  min = ft_pb.min();
}
TransPtr flinTrans::getInverseTransformation() {
  throw(domain_error("inverse function not defined!"));
};
void flinTrans::setTransformedScale(int scale) {
  throw(domain_error("setTransformedScale function not defined!"));
};
...
logicleTrans::logicleTrans(double T, double W, double M, double A, bool _isGml2,
                           int bins, bool isInverse)
    : transformation(false, LOGICLE) {
  calTbl.setInterpolated(true);
  if (T <= 0)
 ...
 */
double
logicleTrans::R_zeroin(           /* An estimate of the root */
...
double
logicleTrans::R_zeroin2(           /* An estimate of the root */
                        double ax, /* Left border | of the range	*/
                        double bx, /* Right border| the root is seeked*/
...
TransPtr logicleTrans::clone() const {
  return TransPtr(new logicleTrans(*this));
};
...
logicleTrans::logicleTrans(const pb::transformation &trans_pb)
    : transformation(trans_pb) {
