...
void MemCytoFrame::parse_fcs_header(ifstream &in, int nOffset) {
  ...
  if (strcmp(version, "FCS2.0") != 0 && strcmp(version, "FCS3.0") != 0 &&
      strcmp(version, "FCS3.1") != 0)
    throw(domain_error(
...
void MemCytoFrame::string_to_keywords(string txt, bool emptyValue) {
  ...
  // FCS 3.1 states only 0-126 ASCII are legal delimiter, but we can't assume
  // the file always follows the standard also the TEXT main contain some
  // special characters , thus we want to make sure the replacement char is not
  // used anywhere in FCS TEXT
  ...
  if (oddChar == 255)
    throw(
        domain_error("Can't find the unused odd character from ASCII(127-255) "
                     "in FSC TEXT section!"));
  ...
  for (unsigned i = 1; i <= j;
    ...
    if (!emptyValue) {
       ...
       * (this slows down things quite a bit, but still a lot faster than R
       * version, and this double delimiter logic is not normally invoked
       * anyway)
       */
    ...
    if ((i) % 2 == 1) {
      if (token.empty())
        // Rcpp::stop (temporarily switch from stop to range_error due to a bug
        // in Rcpp 0.12.8)
  ...
  if (j % 2 == 1) {
    ...
    PRINT(serror + "\n");
    PRINT(
        "The last keyword is dropped.!\nIf it is due to the double delimiters "
        "in keyword value, please set emptyValue to FALSE and try again!");
  }
}
...
MemCytoFrame::MemCytoFrame(const string &filename, const FCS_READ_PARAM &config)
    : filename_(filename), config_(config) {
  set_pheno_data("name", path_base_name(filename));
}
void MemCytoFrame::convertToPb(pb::CytoFrame &fr_pb, const string &h5_filename,
 ...
 */
void MemCytoFrame::read_fcs_data(ifstream &in,
                                 const FCS_READ_DATA_PARAM &config) {
  if (g_loglevel >= GATING_HIERARCHY_LEVEL)
  ...
  if (transDefinedinKeys)
    if (keys_["transformation"] == "applied" ||
        keys_["transformation"] == "custom")
      isTransformation = false;
  ...
  fcsPnGtransform =
      keys_.find("flowCore_fcsPnGtransform") != keys_.end() &&
      keys_["flowCore_fcsPnGtransform"] == "linearize-with-PnG-scaling";
  ...
  if (!multiSize) {
    if (params[0].PnB == 10) {
      ...
      if (sys != "cxp")
        PRINT(
            "Invalid bitwidth specification.\nThis is a known bug in Beckman "
  ...
  //
  //
  //		  //# since signed = FALSE is not supported by readBin when size
  //>
  // 2
  ...
  //
  //	if(dattype != "i")
  //	  throw(domain_error("we don't support different bitwdiths for numeric
  // data type!")); total bits for each row
  size_t nRowSize = accumulate(params.begin(), params.end(), 0,
                               [](size_t i, cytoParam p) { return i + p.PnB; });
  ...
  if (nSelected > 0) {
    if (nSelected >= nrow)
      throw(domain_error(
          "total number of which.lines exceeds the total number of events: " +
          to_string(nrow)));
  ...
  if (nSelected > 0) {
    ...
    for (auto i : which_lines) {
      ...
      if (pos > header_.dataend || pos < header_.datastart)
        throw(domain_error(
            "the index of which.lines exceeds the data boundary: " +
            to_string(i)));
      in.seekg(pos);
                                        ...
                                        // may have extra bytes than needed
    {
      throw(domain_error("file " + filename_ +
                         ...
                         "cells in data section (" +
                         to_string(events_read) +
                         ") is not consistent with keyword '$TOT' (" +
                         to_string(events_expected) + ")"));
    }
  }
  //	nEvents = nrow;
  ...
  //	char *p = buf.get();//pointer to the current beginning byte location of
  // the processing data element in the byte stream
  ...
  if (endian == endianType::mixed) {
    if (multiSize)
      throw(domain_error(
          "Cant't handle diverse bitwidths while endian is mixed: " +
          byte_order));
  ...
  if ((is_host_big_endian() && endian == endianType::small) ||
      (!is_host_big_endian() && endian == endianType::big))
    isbyteswap = true;

    /**
     * cp raw bytes(row-major) to a 2d mat (col-major) represented as 1d
     * array(with different byte width for each elements)
     */
...
#pragma omp parallel for
  for (auto c = 0; c < nCol; c++) {
    ...
    size_t bits_offset =
        accumulate(params.begin(), params.begin() + c, 0,
                   [](size_t i, cytoParam p) { return i + p.PnB; });
    ...
    for (size_t r = 0; r < nrow; r++) {
      ...
      //				# J.Spidlen, Nov 13, 2013: added the
      // flowCore_fcsPnGtransform keyword, which is
      // # set to "linearize-with-PnG-scaling" when
      ...
      if (isTransformation) {
        if (param.PnE[0] > 0) {
          outElement =
              pow(10, outElement / param.max * param.PnE[0]) * param.PnE[1];
    ...
    if (keys_.find("transformation") != keys_.end() &&
        keys_["transformation"] == "custom")
      param.min = boost::lexical_cast<EVENT_DATA_TYPE>(
          keys_["flowCore_$P" + pid + "Rmin"]);
    else {
  ...
  // endl; 	 cout << (omp_get_wtime() - start) / (double)(CLOCKS_PER_SEC /
  // 1000) << endl;
  ...
  for (unsigned i = 0; i < params.size(); i++) {
    ...
    if (isTransformation) {
      ...
      params[i].PnE[0] = 0;
      params[i].PnE[1] = 0;
      keys_["flowCore_$P" + pid + "Rmax"] =
          to_string(static_cast<int>(params[i].max + 1));
      keys_["flowCore_$P" + pid + "Rmin"] =
          to_string(static_cast<int>(params[i].min));
    } else
      params[i].max--;
  }
 ...
 * @param config (input) FCS_READ_HEADER_PARAM object gives the parsing
 * arguments for header
 */
void MemCytoFrame::read_fcs_header(ifstream &in,
  ...
  for (int i = 1; i <= n; i++) {
    ...
    if (keys_.find("$NEXTDATA") != keys_.end()) {
      ...
      if (i < n)
        throw(domain_error("Can't find " + boost::lexical_cast<string>(n) +
                           "th dataset in FCS!"));
      break;
    }
  }
  ...
  if (config.nDataset <= 0 && nNextdata > 0) {
    PRINT(
        "The file contains additional data segment%s.\n The default is to read "
        "the first segment only.\nPlease consider setting the 'dataset' "
        "argument.");
  }
  ...
  if (header_.FCSversion >= 3) {
    ...
    //
    //   # Let's not be too strick here as unfortunatelly, some files exported
    //   from FlowJo # are missing the $BEGINDATA and $ENDDATA keywords and we
    ...
    if (keys_.find("$BEGINDATA") == keys_.end()) {
      if (datastart_h != 0) {
        ...
        throw(domain_error(
            ...
            "keyword and the FCS HEADER does not say it either."));
    } else {
    ...
    if (keys_.find("$ENDDATA") == keys_.end()) {
      if (dataend_h != 0) {
        dataend = dataend_h;
        PRINT(
            "warning:Missing the required $ENDDATA keyword! Reading data based "
            "on information in the FCS HEADER only.\n");
      } else
        throw(domain_error(
            "Don't know where the data segment ends, there was no $ENDDATA "
            "keyword and the FCS HEADER does not say it either."));
    } else {
    ...
    if (datastart_h != datastart) {
      if (datastart_h == 0) //#use the TEXT when header_ is 0
        ...
        msg.append(boost::lexical_cast<string>(header_.datastart) + ":" +
                   boost::lexical_cast<string>(datastart) +
                   ") to read the data.");
    ...
    if (dataend_h != dataend) {
      if (dataend_h == 0 ||
        ...
        msg.append(boost::lexical_cast<string>(header_.dataend) + ":" +
                   boost::lexical_cast<string>(dataend) +
                   ") to read the data.");
        if (config.ignoreTextOffset) {
          msg.append(" The values in TEXT are ignored!\n");
          PRINT(msg);
  ...
  if (isDuplicate) {
    PRINT(
        "channel_alias: Multiple channels from one FCS are matched to the same "
        "alias!\n"
        "Integer suffixes added to disambiguate channels.\n"
        "It is also recommended to verify correct mapping of spillover matrix "
        "columns.\n");
    for (auto chnl : chnls) {
      if (chnl.second.size() > 1) {
        ...
        while (!chnl.second.empty()) {
          ...
          chnl.second.pop();
          params[dup_idx].channel =
              params[dup_idx].channel + "-" + to_string(suffix++);
        }
      }
    }
  }
...
void MemCytoFrame::transform_data(const trans_local &trans) {
  ...
  for (vector<string>::iterator it1 = channels.begin(); it1 != channels.end();
    ...
    auto param_range =
        get_range(curChannel, ColType::channel, RangeType::instrument);
