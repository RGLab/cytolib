...
void check_sample_guid(const string &sample_guid) {
  if (regex_search(sample_guid, regex("(/|\\\\)")))
    throw(domain_error(
        "invalid sample_guid (containing '/' or '\\\\' character): " +
        sample_guid));
}
string fs_tmp_path() { return fs::temp_directory_path().string(); }
...
void recursive_copy(const fs::path &src, const fs::path &dst) {
  ...
  if (fs::is_directory(src)) {
    ...
    for (const fs::directory_entry &item : fs::directory_iterator(src)) {
