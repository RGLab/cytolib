 ...
 * that can now be solved efficiently for t(X) by back substitution, then
 * transposed for X
 */
...
void CytoFrame::scale_time_channel(string time_channel) {
  ...
  if (idx >= 0) {
    ...
    auto param_range =
        get_range(time_channel, ColType::channel, RangeType::data);
...
void CytoFrame::append_columns(const vector<string> &new_colnames,
  ...
  if ((new_colnames.size() <= 0) || (new_colnames.size() != new_cols.n_cols))
    throw(domain_error(
        "Must have equal (nonzero) number of new column names and columns."));
  if (new_cols.n_rows != n_rows())
    throw(domain_error(
        "New columns must have same number of rows as existing columns."));
  ...
  if (std::unique(check_names.begin(), check_names.end()) != check_names.end())
    throw(domain_error("Duplicate new channel names detected. New channel "
  ...
  if (new_cols.n_rows > 0) {
    ...
    // In the absence of data, set the new maxima/minima to +/- infinity to
    // match fr_append_cols
  ...
  for (unsigned i = 0; i < num_toadd; i++) {
    ...
    if (std::isfinite(new_maxs(i)))
      set_keyword("$P" + pid + "R", boost::lexical_cast<string>(
                                        (long long)ceil(new_maxs(i)) + 1));

    if (keys_.find("transformation") != keys_.end() &&
        keys_["transformation"] == "custom") {
      set_keyword("flowCore_$P" + pid + "Rmin",
                  boost::lexical_cast<string>(new_mins(i)));
      set_keyword("flowCore_$P" + pid + "Rmax",
                  boost::lexical_cast<string>(new_maxs(i)));
    }
...
CompType
CytoFrame::get_h5_datatype_params(DataTypeLocation storage_type) const {
  ...
  param_type.insertMember("channel", HOFFSET(cytoParam_cstr, channel),
                          str_type);
  ...
  param_type.insertMember("PnB", HOFFSET(cytoParam_cstr, PnB),
...
void CytoFrame::write_h5_params(H5File file) const {
  ...
  DataSet ds = file.createDataSet(
      "params", get_h5_datatype_params(DataTypeLocation::H5), dsp_param, plist);
...
void CytoFrame::write_h5(const string &filename) const {
  ...
  DataSet dataset = file.createDataSet(
      DATASET_NAME, h5_datatype_data(DataTypeLocation::H5), dataspace, plist);
  /*
...
int CytoFrame::get_col_idx(const string &colname, ColType type) const {
  if (!is_hashed())
    throw(domain_error("please call buildHash() first to build the hash map "
                       "for column index!"));
...
void CytoFrame::set_marker(const string &channelname,
  ...
  if (oldmarkername != markername) { // skip duplication check at this level
                                     // because we need to handle rotation case
    //			if(markername.size() > 0)
    //			{
    //				int id1 = get_col_idx(markername,
    //ColType::marker); 				if(id1>=0&&id1!=id) 					throw(domain_error("marker already
    //exists: " + markername));
    //			}
 ...
 * @param new_range
 */
void CytoFrame::set_range(const string &colname, ColType ctype,
  ...
  if (is_update_keywords) {
    ...
    set_keyword("flowCore_$P" + pid + "Rmin",
                boost::lexical_cast<string>(new_range.first));
    set_keyword("flowCore_$P" + pid + "Rmax",
                boost::lexical_cast<string>(new_range.second));
  }
}
/**
...
CytoFrame::get_range(const string &colname, ColType ctype,
...
/**
 * Compute the time step from keyword either "$TIMESTEP" or "$BTIM", "$TIMESTEP"
 * is preferred when it is present This is used to convert time channel to the
 * meaningful units later on during data transforming
 * @param
 * @return
 */
