...
compensation::compensation(mat spillMat, vector<string> _markers) {
  if (spillMat.n_cols != spillMat.n_rows)
    throw(domain_error("Non-square shaped spillover matrix only possible if "
                       "both marker names and detector names are provided!"));
  if (spillMat.n_cols != _markers.size())
    throw(domain_error("The number of columns of the spillover matrix is not "
                       "the same as the markers!"));
...
compensation::compensation(mat spillMat, vector<string> _markers,
  ...
  if (spillMat.n_cols > spillMat.n_rows)
    throw(
        domain_error("Invalid spillover matrix (less channels than labels)!"));
  if (spillMat.n_cols != _markers.size())
    throw(domain_error("The number of columns of the spillover matrix is not "
                       "the same as the markers!"));
...
void compensation::update_channels(const CHANNEL_MAP &chnl_map) {
  ...
  for (vector<string>::iterator it = detector.begin(); it != detector.end();
       it++) {
    string curName = *it;
...
compensation::compensation(const pb::COMP &comp_pb)
    : cid(comp_pb.cid()), prefix(comp_pb.prefix()), suffix(comp_pb.suffix()),
      name(comp_pb.name()), comment(comp_pb.comment()) {
