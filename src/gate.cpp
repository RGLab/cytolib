...
void paramPoly::convertToPb(pb::paramPoly &paramPoly_pb){
    BOOST_FOREACH (vector<string>::value_type &it,
                   params){paramPoly_pb.add_params(it);
} // namespace cytolib
...
INDICE_TYPE rangeGate::gating(MemCytoFrame &fdata, INDICE_TYPE &parentInd) {
  ...
  EVENT_DATA_TYPE *data_1d =
      fdata.get_data_memptr(param.getName(), ColType::channel);
...
void rangeGate::extend(MemCytoFrame &fdata, float extend_val) {
  ...
  if (param.getMin() <= extend_val) {
    if (g_loglevel >= POPULATION_LEVEL)
      PRINT("extending " + pName + "from " + to_string(param.getMin()) +
            " to :" + to_string(xMin) + "\n");
 ...
 * losing the data points that are below this theshold range to cut data range)
 */
void polygonGate::extend(MemCytoFrame &fdata, float extend_val) {
  ...
  for (unsigned i = 0; i < v.size(); i++) {
    if (v[i].x <= extend_val) {
      if (g_loglevel >= POPULATION_LEVEL)
        PRINT("extending " + x + "from " + to_string(v[i].x) +
              " to :" + to_string(xMin) + "\n");
      v[i].x = min(xMin, v[i].x);
    }
    if (v[i].y <= extend_val) {
      if (g_loglevel >= POPULATION_LEVEL)
        PRINT("extending " + y + "from " + to_string(v[i].y) +
              " to :" + to_string(yMin) + "\n");
 ...
 *  indices are allocated within gating function, so it is up to caller to free
 * it and now it is freed in destructor of its owner "nodeProperties" object
 */
...
vector<coordinate> ellipseGate::getCovarianceMat() const {
  if (!Transformed())
    throw(domain_error("EllipseGate has not been transformed so covariance "
...
coordinate ellipseGate::getMu() const {
  if (!Transformed())
    throw(domain_error(
        "EllipseGate has not been transformed so mu is unavailable!"));
  return mu;
};
EVENT_DATA_TYPE ellipseGate::getDist() const {
  if (!Transformed())
    throw(domain_error(
        "EllipseGate has not been transformed so dist is unavailable!"));
  return dist;
};
vector<coordinate> ellipseGate::getAntipodalVerts() const {
  return antipodal_vertices;
}
ellipseGate::ellipseGate(coordinate _mu, vector<coordinate> _cov,
                         EVENT_DATA_TYPE _dist)
    : mu(_mu), cov(_cov), dist(_dist) {
  isTransformed = true;
  isGained = true;
  neg = false;
...
ellipseGate::ellipseGate(vector<coordinate> _antipodal, vector<string> _params)
    : antipodal_vertices(_antipodal), dist(1) {
  isTransformed = false;
  isGained = false;
  neg = false;
  setShift(vector<EVENT_DATA_TYPE>{0.0, 0.0});
...
void ellipseGate::computeCov() {
  if (!Transformed())
    throw(domain_error("antipodal points of ellipseGate must be transformed "
                       "before computing covariance matrix!"));
  ...
  // far right point
  vector<coordinate>::iterator R_it = max_element(
      v.begin(), v.end(), [](coordinate i, coordinate j) { return i.x < j.x; });
  coordinate R = *R_it;

  // far left point
  vector<coordinate>::iterator L_it = min_element(
      v.begin(), v.end(), [](coordinate i, coordinate j) { return i.x < j.x; });
  coordinate L = *L_it;
...
INDICE_TYPE ellipseGate::gating(MemCytoFrame &fdata, INDICE_TYPE &parentInd) {
  ...
  EVENT_DATA_TYPE *xdata =
      fdata.get_data_memptr(param.xName(), ColType::channel);
  EVENT_DATA_TYPE *ydata =
      fdata.get_data_memptr(param.yName(), ColType::channel);
  ...
  for (auto i : parentInd) {
    ...
    bool isIn =
        (x * x * aa + x * y * cc + x * y * bb + y * y * dd) <= pow(dist, 2);
    if (isIn != neg)
      res.push_back(i);
  }
...
ellipseGate::ellipseGate(const pb::gate &gate_pb)
    : polygonGate(gate_pb), mu(coordinate(gate_pb.eg().mu())),
      dist(gate_pb.eg().dist()) {
...
void ellipseGate::interpolatePolygon(unsigned nVertices = 50) {
  ...
  Row<EVENT_DATA_TYPE> theta =
      linspace<Row<EVENT_DATA_TYPE>>(0.0, 2.0 * datum::pi, nVertices);
...
void ellipsoidGate::transforming(trans_local &trans) {
  ...
  if (!Transformed()) {
    ...
    /*
     * re-construct the trans object that was used by flowJo to transform
     * ellipsoid gate to 256 scale
     */
    TransPtr trans_gate_x, trans_gate_y;
    if (!trans_x)
      throw(domain_error(
          "ellipsoidGate::transforming can't find transformation for " +
          channel_x));
    //			trans_gate_x.reset(new scaleTrans()); //create default scale trans
    //for linear, assuming the max value for linear scale is always 262144 		else
    ...
    if (!trans_y)
      throw(domain_error(
          "ellipsoidGate::transforming can't find transformation for " +
          channel_y));
    //			trans_gate_y.reset(new scaleTrans()); //create default scale trans
    //for linear 		else
     ...
     */
    isTransformed = false; // reset transform flag otherwise the transforming
                           // won't get executed
...
void CurlyQuadGate::transforming(trans_local &trans) {
  if (interpolated)
    ...
    throw(logic_error(
        "CurlyQuadGate can't not be transformed before interpolation!"));
};
...
INDICE_TYPE CurlyQuadGate::gating(MemCytoFrame &fdata, INDICE_TYPE &parentInd) {
  if (interpolated) {
    ...
    throw(logic_error(
        "CurlyQuad gate has not been converted to polygonGate yet!"));
  }
}
...
void CurlyQuadGate::interpolate(trans_local &trans) {
  ...
  if (!trans_x)
    trans_gate_x.reset(
        new scaleTrans()); // create default scale trans for linear, assuming
                           // the max value for linear scale is always 262144
  else
    trans_gate_x = trans_x->clone(); // copy existing trans_x for non-linear

  if (!trans_y)
    trans_gate_y.reset(
        new scaleTrans()); // create default scale trans for linear
  else
    trans_gate_y = trans_y->clone(); // copy existing trans_y for non-linear
  ...
  setTransformed(
      false); // reset flag so that it won't interfere the next transforming
  ...
  EVENT_DATA_TYPE x_min =
      -4e3; //-numeric_limits<EVENT_DATA_TYPE>::max();//xdata.min();
  EVENT_DATA_TYPE y_min =
      -4e3; //-numeric_limits<EVENT_DATA_TYPE>::max();//ydata.min();
