	...
	GatingSet::GatingSet(string path, bool is_skip_data, bool readonly)
	{
		...
		if (!input) {
			...
			for(int i = 0; i < pbGS.samplename_size(); i++){
				...
				string h5_filename = (fs::path(path) / (sn + ".h5")).c_str();
...
//			}
	...
	GatingSet GatingSet::copy(bool is_copy_data, bool is_realize_data, const string & new_h5_dir) const{
		...
		fs::path h5_dir;
		if(is_copy_data)
			h5_dir = gs.generate_h5_folder(fs::path(new_h5_dir).c_str());
		for(const string & sn : get_sample_uids())
		{
			...
			gs.add_GatingHierarchy(gh->copy(is_copy_data, is_realize_data, (h5_dir/sn).c_str()), sn);
	...
	{
		h5_dir = (fs::path(h5_dir) / uid_).c_str();
		if(fs::exists(h5_dir))
