 ...
 * the user of this software. MemCytoFrame.hpp
 *
...
/**
 * The class represents the in-memory version of CytoFrame, which stores and
 * owns the events data
 */
class MemCytoFrame : public CytoFrame {
  EVENT_DATA_VEC data_; // col-major
  vector<string> rownames_;
  // below are cached for fcs parsing, should be of no usage once the data
  // section is parsed
  string filename_;
  FCS_READ_PARAM config_;
  FCS_Header header_;
  ifstream in_; // because of this member, the class needs to explicitly define
...
public:
   ...
   * @param onlyTxt flag indicates whether to only parse text segment (which
  ...
  void convertToPb(pb::CytoFrame &fr_pb, const string &h5_filename,
                   CytoFileOption h5_opt, const CytoCtx &ctx = CytoCtx()) const;
  ...
  FileFormat get_backend_type() const { return FileFormat::MEM; };
  /**
   ...
   * @return
   */
  vector<string> get_rownames() const { return rownames_; }
  void set_rownames(const vector<string> &data_in) {
    if (n_rows() != data_in.size())
      throw(domain_error(
          "the input rownames size is different from the matrix size!"));
    rownames_ = data_in;
  }
  void del_rownames() { rownames_.clear(); }
  EVENT_DATA_VEC get_data() const { return data_; }
  EVENT_DATA_VEC &get_data_ref() { return data_; }
   ...
   * @param _data
   */
  void set_data(const EVENT_DATA_VEC &_data) { data_ = _data; }
  /**
   * move setter
   * @param _data
   */
  void set_data(EVENT_DATA_VEC &&_data) { swap(data_, _data); }
  /**
