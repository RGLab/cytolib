 ...
 * the user of this software. GatingHierarchy.hpp
 *
...
typedef vector<pair<VertexID, VertexID>>
    VertexID_edge_vec; // represents edges as <start, end> pairs
...
struct OurVertexPropertyWriterR {
  ...
  template <class Vertex> void operator()(std::ostream &out, Vertex u) {
    ...
    out << "[shape=record,label=\"" << curNode.getName()
        << "\",isBool=" << isBoolGate << ",hidden=" << hidden << "]";
  }
 ...
 **
 ** It stores the transformation, compensation and gating information as well as
 the flow data Each FCS file is associated with one GatingHierarchy object. It
 can also serves as a gating template when data is empty.
 */
class GatingHierarchy {
private:
  ...
  /* compensation is currently done in R due to the linear Algebra
                                          e[, cols] <-
     t(solve(t(spillover))%*%t(e[,cols])) we can try uBlas for this simple task,
     but when cid=="-1",we still need to do this in R since comp is extracted
     from FCS keyword (unless it can be optionally extracted from workspace
     keyword)
                                    */
  populationTree tree; /**< the gating tree */

  PARAM_VEC transFlag; /*< for internal use of parse flowJo workspace */
  trans_local trans;   /*< the transformation used for this particular
...
public:
  ...
  void set_cytoframe_view(CytoFrameView fr) { frame_ = fr; }
  /*
  ...
  vector<string> get_markers() const {
    return get_cytoframe_view().get_markers();
  };
  vector<string> get_channels() const {
    return get_cytoframe_view().get_channels();
  };
  void set_marker(const string &_channel, const string &_marker) {
    get_cytoframe_view_ref().set_marker(_channel, _marker);
  }
  void set_channel(const string &_old, const string &_new) {
    get_cytoframe_view_ref().set_channel(_old, _new);
  }
  const PDATA &get_pheno_data() const {
    return get_cytoframe_view().get_pheno_data();
  }
  ...
  /**
   * transform the data
   * The reason we pass in MemCytoFrame is because the data member frame_ may
   * not be finalized yet at this stage of parsing.
   */
  void transform_data(MemCytoFrame &cytoframe);

  void calgate(MemCytoFrame &cytoframe, VertexID u, bool computeTerminalBool,
   ...
   */
  GatingHierarchy() {
    addRoot(); // add default root to avoid the risk of crashing on accessing
               // the empty boost graph
  }
  ...
  GatingHierarchy(compensation _comp, PARAM_VEC _transFlag, trans_local _trans)
      : comp(_comp), transFlag(_transFlag), trans(_trans) {
    addRoot();
  };
  /**
   *
   ...
   * typically remain as default unless for debug purpose (e.g. re-writing gs
   * that is loaded from legacy pb archive without actual data associated)
   */
  void convertToPb(pb::GatingHierarchy &gh_pb, string uri,
                   CytoFileOption h5_opt, bool is_skip_data = false,
                   const CytoCtx &ctx = CytoCtx());
  GatingHierarchy(CytoCtx ctx, pb::GatingHierarchy &pb_gh, string uri,
                  bool is_skip_data, bool readonly = true) {
    ...
    for (int i = 0; i < nNodes; i++) {
      ...
      // check if current node is root (since parent could be 0 for both /root
      // and root/A for proto3 when its parent is absent) assumption is node A
      // won't be named as 'root', will be enforced at name setter
   ...
   * gate assuming gate split the parent population into two subpops, one of
   * which is to keep depends on isNegate flag of the gate
   */
  VertexID addGate(gatePtr g, VertexID parentID, string popName);
  /*
   * remove the node along with associated population properities including
   * indices and gates can't do recursive removal here since vid is subject to
   * change during the removal precoess
   */
   ...
   * @return
   */
  compensation get_compensation() { return comp; }
   ...
   *
   */
  void gating(MemCytoFrame &cytoframe, VertexID u, bool recompute = false,
              bool computeTerminalBool = true, bool skip_faulty_node = false);
  void gating(MemCytoFrame &cytoframe, VertexID u, bool recompute,
              bool computeTerminalBool, bool skip_faulty_node,
              INTINDICES &parentIndice);
  /*
  ...
  vector<bool> boolGating(MemCytoFrame &cytoframe, VertexID u,
   ...
   * @return
   */
  vector<bool> boolGating(MemCytoFrame &cytoframe,
                          vector<BOOL_GATE_OP> boolOpSpec,
                          bool computeTerminalBool);

  /*
   * current output the graph in dot format
   * and further covert it to gxl in order for Rgraphviz to read since it does
   * not support dot directly right now the data exchange is through file
   * system,it would be nice to do it in memory
   */
   ...
   *
   * @param order accept 3 values: REGULAR(0) is the same original order by
   * which nodes were added; TSORT(1) topological order; BFS(2) breadth first
   * searching order
   ...
   * @return
   */
  static bool isEmpty(string path) { return (path.empty()); }
  /**
  ...
  /*
   * retrieve the VertexID by the gating path.
   * this serves as a parser to convert generic gating path into internal node
   * ID and it doesn't allow ambiguity (throw the exception when multiple nodes
   * match)
   ...
   * @param nDepths the depths of ancestor node. It is used to as the
   * measurement to determine which reference node to win when multiple matches
   * occur. The deeper it is, the nearer the reference is to the boolean node.
   * @return ancestor ID
   */
  ...
  /**
   * search for all the nodes that matches the pop name given the ancestor node
   id
   ...
   * It is used to search for the first node in the gating path (full or
   * partial). This is different from getRefNodeID in the way that pop name must
   * be uniquely identifiable in the tree.
   ...
   * @return
   */
  vector<string> getNodePaths(unsigned short order, bool fullPath,
                              bool showHidden);
  ...
  /*
   * Retrieve a representation of the tree as a vector of directed edges
   * represented as pairs of VertexIDs and a vector of VertexIDs of the leaf
   * nodes (aka "tips").
   *
  ...
  GatingHierarchyPtr copy(bool is_copy_data, bool is_realize_data,
                          const string &uri) const;
  /*
   * It is mainly used by Rcpp API addTrans to propagate global trans map to
   * each sample EDIT: But now also used by clone methods
   * @param trans trans_map
   */
  void addTransMap(trans_map tm) { trans.setTransMap(tm); }
};
}; // namespace cytolib
