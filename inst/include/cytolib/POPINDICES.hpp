 ...
 * the user of this software. POPINDICES.hpp
 *
 ...
 *
 *  It is an abstract class that hides the details of implementation of the
 * actual indices type.
 */
class POPINDICES {
protected:
...
public:
  ...
  vector<bool> getIndices() { return x; }
  vector<unsigned> getIndices_u();

  unsigned getCount() { return count(x.begin(), x.end(), true); }
...
public:
  ...
  INTINDICES(vector<unsigned> _ind, unsigned _nEvent)
      : POPINDICES(_nEvent), x(_ind){};

  vector<bool> getIndices();

  vector<unsigned> getIndices_u() { return x; };
  unsigned getCount() { return x.size(); }
...
public:
  ...
  unsigned getCount() { return nEvents; }
  ...
  ROOTINDICES(const pb::POPINDICES &ind_pb) { nEvents = ind_pb.nevents(); }
};
