 ...
 * the user of this software. readFCSHeader.hpp
 *
...
public:
  ...
  iterator find(const string &key) {
    return std::find_if(
        kw.begin(), kw.end(),
        [key](const pair<string, string> &p) { return p.first == key; });
  }
  ...
  pair<string, string> &operator[](const int &n) { return kw[n]; }
...
struct FCS_READ_HEADER_PARAM {
  ...
  bool isEmptyKeyValue; // whether allow the keyword value to be empty. When
                        // true, then double delimiter will be considered as
                        // empty keyword value.
  bool ignoreTextOffset; // whether to ignore the offset recorded in the text
                         // segment of FCS
  int nDataset; // which data set to be parsed when multi-data segments are
                // detected
...
/**
 * parameters parsed from keywords, but may be accessed frequently later thus
 * saved as the copy in this struct for fast and easy query
 */
struct cytoParam {
  string channel, marker;
  EVENT_DATA_TYPE min, max, PnG;
  //	pair<EVENT_DATA_TYPE, EVENT_DATA_TYPE> PnE;//replace pair with simple
  //array since it is not clear how to create compound type for pair
